...
class SkillsLoader:
    ...
    def _read_enumerated_skill_bytes(self, path: Path, within: str | None) -> bytes | None:
        ...
        try:
            raw = safe_read_file_bytes_nolink(str(path), within_root=within)
        except FileTooLargeError:
        ...
        *project_dir* additionally allows a body to come from that project's own
        ...
        if project_key:
            ...
            for candidate, skill_file, _within in self._iter(project_dir):
                ...
                # unchecked.
                raw = self._read_enumerated_skill_bytes(skill_file, _within)
                if raw is None:
                    logger.warning(
    ...
    def _redact_text(text: object) -> str:
        ...
        Project catalog descriptions and pending skill detail/approval both
        reach the dashboard from files an untrusted producer can write. Apply
        the same exfiltration-URL and credential passes at those read points so
        neither surface can return secrets or promote them live.
        """
    ...
    def get_context(
        ...
        this is the "core" set (mark core skills ``always: true`` to pin them).
        Confined project skills also get full content because exposing their
        mutable checkout paths would let a later direct read bypass confinement.
        The remaining unconfined on-demand skills are ranked by usage (hottest
        ...
        unconfined summary block so no single section can blow the context
        budget.
        ...
        *only* restricts the block to skills whose ``SKILL.md`` path matches one
        ...
        # Pinned and confined project skills: full content, always injected.
        ...
        for s in all_skills:
            if s["key"] not in pinned and not s.get("confine_root"):
                continue
    ...
    def _legacy_context(
        ...
        Full content for pinned (``always: true``) and confined project skills +
        a one-line summary for every unconfined on-demand skill, unranked and
        untruncated. Project bodies replace their unsafe live-path summaries;
        unconfined skills retain the behavior from before lazy loading.
        ...
        *project_dir* is forwarded to the ``repo_scope`` gate so this path
        ...
        # A project skill that is not already pinned still needs body delivery:
        # the alternative is a path the agent can reopen outside confinement.
        for s in all_skills:
            if not s.get("confine_root") or s["key"] in always:
                continue
            content = self.load_skill(s["key"], project_dir)
            if content:
                stripped = self.strip_frontmatter(content)
