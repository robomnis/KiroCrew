...
def normalize_pull_request_target(
    ...
    *,
    ...
    if kind == "github_pull_request":
        return parse_github_pull_request_target(raw).url
    if kind == "gitlab_merge_request":
        return parse_gitlab_merge_request_target(raw, gitlab_hosts=gitlab_hosts).url
    if kind == "azure_devops_pull_request":
        return parse_azure_devops_pull_request_target(raw).url
    if kind == "bitbucket_pull_request":
        return parse_bitbucket_pull_request_target(raw).url
