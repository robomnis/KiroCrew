...
def _withheld_env_count(
    ...
    disqualifies the entry from pooling (issue #3495 cause B). With forwarding
...
def _build_stub_entry(
    *,
    ...
    ENOENT (issue #3495 cause A) or whose declared env is silently dropped
    (cause B).
...
def rewrite_agents(
    *,
    ...
    # (and ``notes.sidecar_write_failed`` makes the pass uncacheable). One arm
    # needs a pass-level guard: an agent spec whose READ failed never ran
    # ``_rewrite_single_spec``, so its sidecar names are unknowable this pass
    # — and its KEPT overlay still points ``--env-file`` at them; pruning
    ...
    env_dir = env_sidecar_dir_for_stubs(stubs_dir)
    if env_dir.is_dir() and not notes.source_read_failed:
        for stale in env_dir.glob("*.json"):
