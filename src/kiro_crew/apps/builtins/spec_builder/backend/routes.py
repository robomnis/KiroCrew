...
def _clear_duplicate_stage_documents_at(dir_fd: int, token: str, documents: object) -> bool:
    ...
    """
    if not isinstance(documents, dict) or not _duplicate_marker_matches_at(dir_fd, token):
        return False
    names = list(documents)
    if not names or any(name not in _EDITABLE_DOCS for name in names):
        return False
    try:
        for fname in names:
            try:
...
async def _handle_duplicate(request: web.Request) -> web.Response:
    ...
    async def _complete_transaction() -> tuple[str, str, Path]:
        ...
        if not await _mutate_index(_finish):
