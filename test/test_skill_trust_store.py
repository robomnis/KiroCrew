...
class TestConsentCoversOnlyTheGrantedDirectory:
    ...
    def test_confined_walk_stops_before_python_recursion_can_fail(self, monkeypatch):
        ...
        monkeypatch.setattr(skills_mod.os, "scandir", lambda _fd: FakeScanner())
        monkeypatch.setattr(skills_mod.os, "open", open_child)
        monkeypatch.setattr(skills_mod.os, "close", lambda _fd: None)
...
class TestCatalogOnlyOffersLoadableWorkspaceSkills:
    ...
    def test_project_descriptions_are_redacted_before_reaching_the_dashboard(
        ...
        (skill_dir / "SKILL.md").write_text(
            f"---\nname: hostile\ndescription: {credential} {exfil_url}\n---\n",
            encoding="utf-8",
        )
        loader = SkillsLoader(skills_path=tmp_path / "home-skills", install_builtins=False)
