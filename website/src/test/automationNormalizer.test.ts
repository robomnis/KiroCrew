...
describe('automation transport normalizer', () => {
  ...
  it('fails closed when canonical evidence belongs to another provider kind', () => {
  ...
  it('fails closed when canonical check-name evidence exceeds backend bounds', () => {
    const record = normalizeAutomationRecord(structuredLoop({
      last_observation: {
        ...canonicalGitHubObservation,
        checks: {
  ...
  it.each([
    ...
    ['observation summary', { last_observation_summary: false }],
  ...
  ])('fails closed for malformed canonical pull-request %s facts', (_name, lastObservation) => {
    const record = normalizeAutomationRecord(structuredLoop({
  ...
  it('reports a removed nonterminal structured record as blocked, never active', () => {
    const record = normalizeAutomationRecord({
