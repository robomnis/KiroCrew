...
child starts outside the agent-controlled workspace. GitLab current-head pipeline
and discussion evidence reads at most two pages before remaining explicitly
incomplete. Error mapping is shared: rate limits and transport failures retry without
...
GitLab maps current-head pipelines, approval state, unresolved discussions, and detailed
merge status into the shared facts. Azure maps pull-request status, source commit,
...
probe or model turn; `UNAVAILABLE` is terminal. If dispatch fails before a turn
starts, no agent-turn or token budget is charged. Stream exhaustion is dispatch,
not completion. If the evidence deadline expires without a raw completion event,
the monitor atomically retains `completion_evidence_unavailable`, clears the
claim, and fails closed rather than issuing a duplicate wake. Restart resumes a
persisted `BUSY` claim at its existing retry deadline. Cadence edits affect only
...
started turn owns the correlated completion hook.
...
- Targets are normalized and validated against the closed provider/host/path
  matrix; no adapter accepts arbitrary URLs or commands.
