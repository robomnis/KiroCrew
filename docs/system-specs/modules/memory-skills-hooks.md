...
budgeted initial skills blocks inject every admitted project skill body through
...
and persuading the agent to reopen it directly after the descriptor-confined read.
...
Because the description is checked-out, untrusted text rendered by the dashboard, it is
passed through both the exfiltration-URL and credential redactors before leaving the
backend.
